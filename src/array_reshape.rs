...
pub fn image_to_line<'a>(image: &'a ArrayView3<'a, u8>) -> Array2<u8> {
    ...
    let mut flat_img: Array2<u8> = Array2::zeros(
        (long_side.pow(2), im_shape[2])
    );
...
pub fn line_to_image<'a>(image: &'a ArrayView2<'a, u8>, imshape1: usize, imshape2: usize) -> Array3<u8> {
    let mut out_img: Array3<u8> = Array3::zeros((
        imshape1, imshape2, image.shape()[1]
    ));
...
mod tests {
    ...
    fn image_to_line_to_image_square() {
        ...
        let arr: Array3<u8> = Array3::from_shape_fn(
            (sh1, sh2, sh3),
            |(a, b, c)| odd_func(a, b, c),
        );
